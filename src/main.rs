...
async fn index(xsize: i32, ysize: i32, bomb_amount: i32, games: &State<Games>) -> String {
    if xsize > 0 && xsize < 100 && ysize > 0 && ysize < 100 && bomb_amount > 0 && bomb_amount < 100*100 {
...
async fn action(id: usize, action: String, xpos: i32, ypos: i32, games: &State<Games>) -> String{
    ...
    if Option::is_some(&game) {
        ...
        */
        let mut res = vec![vec![0u8; game.board_size.x.try_into().unwrap()]; game.board_size.y.try_into().unwrap()];
